export default function Seats() {
  return (
    <div className="w-[1440px] h-[1024px] relative bg-black">
      <div className="w-[1358px] h-[783px] left-[47px] top-[137px] absolute">
        <div className="w-[200px] h-[226px] left-0 top-[84px] absolute">
          <div className="w-[200px] left-0 top-0 absolute">
            <span className="text-orange-500 text-[32px] font-bold font-['Maven Pro'] leading-normal">
              Movie:{" "}
            </span>
            <br/>
            <span className="text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
              Iron Man
            </span>
          </div>
          <div className="w-[200px] left-0 top-[102px] absolute">
            <span className="text-orange-500 text-[32px] font-bold font-['Maven Pro'] leading-normal">
              Showtime:{" "}
            </span>
            <br/>
            <span className="text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
              10/10    1:00 PM
            </span>
          </div>
        </div>
        <div className="w-[1151px] h-[779px] left-[207px] top-[4px] absolute">
          <div className="w-[1151px] h-[739px] left-0 top-[40px] absolute bg-zinc-700 bg-opacity-50 rounded-[17px]" />
          <div className="w-[351px] h-[660px] left-[787px] top-0 absolute">
            <div className="w-[341px] h-[303px] left-[5px] top-[69px] absolute">
              <div className="w-[341px] h-9 left-0 top-[61px] absolute">
                <div className="w-[78px] h-9 left-0 top-0 absolute">
                  <div className="w-[74px] h-[18px] left-[4px] top-[7px] absolute text-zinc-200 text-opacity-80 text-[15px] font-medium font-['Maven Pro'] leading-normal">
                    Adult
                  </div>
                </div>
                <div className="left-[90px] top-[4px] absolute">
                  <span className="text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    Seat:{" "}
                  </span>
                  <span className="text-white text-2xl font-normal font-['Maven Pro'] leading-normal">
                    A3
                  </span>
                </div>
                <div className="left-[196px] top-[4px] absolute text-right">
                  <span className="text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    Price: $
                  </span>
                  <span className="text-white text-2xl font-normal font-['Maven Pro'] leading-normal">
                    15.00
                  </span>
                </div>
              </div>
              <div className="w-[200px] h-[0px] left-[271px] top-[113px] absolute origin-top-left -rotate-180 border border-black"></div>
              <div className="w-[331px] h-9 left-0 top-[130px] absolute">
                <div className="w-[78px] h-9 left-0 top-0 absolute">
                  <div className="w-[74px] h-[18px] left-[4px] top-[7px] absolute text-zinc-200 text-opacity-80 text-[15px] font-medium font-['Maven Pro'] leading-normal">
                    Child
                  </div>
                </div>
                <div className="left-[90px] top-[4px] absolute">
                  <span className="text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    Seat:{" "}
                  </span>
                  <span className="text-white text-2xl font-normal font-['Maven Pro'] leading-normal">
                    C4
                  </span>
                </div>
                <div className="left-[196px] top-[4px] absolute">
                  <span className="text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    Price: $
                  </span>
                  <span className="text-white text-2xl font-normal font-['Maven Pro'] leading-normal">
                    5.00
                  </span>
                </div>
              </div>
              <div className="w-[200px] h-[0px] left-[271px] top-[182px] absolute origin-top-left -rotate-180 border border-black"></div>
              <div className="w-[341px] h-9 left-0 top-[198px] absolute">
                <div className="w-[78px] h-9 left-0 top-0 absolute">
                  <div className="w-[74px] h-[18px] left-[4px] top-[7px] absolute text-zinc-200 text-opacity-80 text-[15px] font-medium font-['Maven Pro'] leading-normal">
                    Senior
                  </div>
                </div>
                <div className="left-[91px] top-[4px] absolute">
                  <span className="text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    Seat:{" "}
                  </span>
                  <span className="text-white text-2xl font-normal font-['Maven Pro'] leading-normal">
                    C5
                  </span>
                </div>
                <div className="left-[194px] top-[4px] absolute">
                  <span className="text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    Price: $
                  </span>
                  <span className="text-white text-2xl font-normal font-['Maven Pro'] leading-normal">
                    10.00
                  </span>
                </div>
              </div>
              <div className="w-[200px] h-[0px] left-[271px] top-[251px] absolute origin-top-left -rotate-180 border border-black"></div>
              <div className="w-[298px] h-9 left-0 top-[267px] absolute">
                <div className="w-[78px] h-9 left-0 top-0 absolute">
                  <div className="w-[74px] h-[18px] left-[4px] top-[7px] absolute text-zinc-200 text-opacity-80 text-[15px] font-medium font-['Maven Pro'] leading-normal">
                    Age
                  </div>
                </div>
                <div className="left-[91px] top-[4px] absolute">
                  <span className="text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    Seat:{" "}
                  </span>
                  <span className="text-white text-2xl font-normal font-['Maven Pro'] leading-normal">
                    C6
                  </span>
                </div>
                <div className="left-[196px] top-[4px] absolute">
                  <span className="text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    Price: ${" "}
                  </span>
                  <span className="text-white text-2xl font-normal font-['Maven Pro'] leading-normal">
                    -{" "}
                  </span>
                </div>
              </div>
              <div className="w-[325px] h-11 left-[4px] top-0 absolute">
                <div className="w-[129px] h-[23px] left-[12px] top-0 absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                  Total Seats:
                </div>
                <div className="w-[164px] h-[23px] left-[141px] top-0 absolute text-right text-white text-2xl font-normal font-['Maven Pro'] leading-normal">
                  4
                </div>
                <div className="w-[325px] h-[0px] left-[325px] top-[44px] absolute origin-top-left -rotate-180 border border-black"></div>
              </div>
            </div>
            <div className="w-[308px] h-0 left-[53px] top-[-25px] absolute text-orange-500 text-5xl font-bold font-['Maven Pro'] leading-normal">
              Select Age
            </div>
          </div>
          <div className="w-[762px] h-[606px] left-[15px] top-[54px] absolute">
            <div className="w-[700px] h-[30px] left-[31px] top-[565px] absolute">
              <div className="w-[700px] h-[30px] left-0 top-0 absolute bg-orange-50" />
              <div className="w-[80.92px] left-[310px] top-[-4px] absolute text-center text-black text-2xl font-bold font-['Maven Pro'] leading-normal">
                Screen
              </div>
            </div>
            <div className="w-[701px] h-[489px] left-[30px] top-[18px] absolute">
              <div className="w-[700px] h-[94px] left-0 top-0 absolute">
                <div className="w-[70px] h-[94px] left-0 top-0 absolute">
                  <div className="left-[22px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    A1
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[70px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    A2
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[140px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    A3
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[210px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    A4
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[280px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    A5
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[350px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    A6
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[420px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    A7
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[490px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    A8
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[560px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    A9
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[630px] top-0 absolute">
                  <div className="left-[14px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    A10
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
              </div>
              <div className="w-[700px] h-[94px] left-[1px] top-[110px] absolute">
                <div className="w-[70px] h-[94px] left-0 top-0 absolute">
                  <div className="left-[22px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    B1
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[70px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    B2
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[140px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    B3
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[210px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    B4
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[280px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    B5
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[350px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    B6
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[420px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    B7
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[490px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    B8
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[560px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    B9
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[630px] top-0 absolute">
                  <div className="left-[14px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    B10
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
              </div>
              <div className="w-[700px] h-[94px] left-[1px] top-[285px] absolute">
                <div className="w-[70px] h-[94px] left-0 top-0 absolute">
                  <div className="left-[22px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    C1
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[70px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    C2
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[140px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    C3
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[210px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    C4
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[280px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    C5
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[350px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    C6
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[420px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    C7
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[490px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    C8
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[560px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    C9
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[630px] top-0 absolute">
                  <div className="left-[14px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    C10
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
              </div>
              <div className="w-[700px] h-[94px] left-[1px] top-[395px] absolute">
                <div className="w-[70px] h-[94px] left-0 top-0 absolute">
                  <div className="left-[22px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    D1
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[70px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    D2
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[140px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    D3
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[210px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    D4
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[280px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    D5
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[350px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    D6
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[420px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    D7
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[490px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    D8
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[560px] top-0 absolute">
                  <div className="left-[20px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    D9
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
                <div className="w-[70px] h-[94px] left-[630px] top-0 absolute">
                  <div className="left-[14px] top-[70px] absolute text-white text-2xl font-medium font-['Maven Pro'] leading-normal">
                    D10
                  </div>
                  <img
                    className="w-[70px] h-[70px] left-0 top-0 absolute"
                    src="https://via.placeholder.com/70x70"
                  />
                </div>
              </div>
            </div>
          </div>
          <div className="left-[27px] top-[743px] absolute text-center text-white text-xl font-normal font-['Maven Pro'] leading-normal">
            Select a seat in “Select Seats”, then apply the age category of each
            seat in “Select Age.”
          </div>
          <div className="w-[396px] h-[28.33px] left-[268px] top-[-25px] absolute text-orange-500 text-5xl font-bold font-['Maven Pro'] leading-normal">
            Select Seats
          </div>
        </div>
        <div className="w-[578px] h-[70px] left-[314px] top-[665px] absolute">
          <div className="left-[265px] top-[23px] absolute text-center text-white text-2xl font-bold font-['Maven Pro'] leading-normal">
            Selected
          </div>
          <div className="left-[445px] top-[23px] absolute text-center text-white text-2xl font-bold font-['Maven Pro'] leading-normal">
            Unavaliable
          </div>
          <div className="left-[77px] top-[23px] absolute text-center text-white text-2xl font-bold font-['Maven Pro'] leading-normal">
            Avaliable
          </div>
          <img
            className="w-[70px] h-[70px] left-[368px] top-0 absolute"
            src="https://www.figma.com/file/REWCHzaLG3PQZZahjMYWSR/Cinema-E-Booking-Prototype?type=design&node-id=76-741&mode=design&t=fD7pXLTsakpKD2sY-4"
          />
          <img
            className="w-[70px] h-[70px] left-[188px] top-0 absolute"
            src="https://via.placeholder.com/70x70"
          />
          <img
            className="w-[70px] h-[70px] left-0 top-0 absolute"
            src="https://via.placeholder.com/70x70"
          />
        </div>
        <div className="w-[102px] h-11 left-[1210px] top-[720px] absolute">
          <button
              className="px-12 py-3 rounded-br-3xl rounded-tl-3xl bg-orange-500 text-white font-bold font-['Maven Pro'] leading-normal transition duration-200 hover:bg-orange-500 hover:text-black border-2 border-transparent ">
            Next
          </button>
        </div>
        <div className="w-[135px] h-11 left-[32px] top-0 absolute">
          <div className="w-[135px] h-11 z-10 left-[-17px] top-[-65px] absolute">
            <button
                className="px-8 py-2 rounded-bl-3xl rounded-tr-3xl bg-orange-500 text-white font-bold font-['Maven Pro'] leading-normal transition duration-200 hover:bg-orange-500 hover:text-black border-2 border-transparent ">
              Back
            </button>
          </div>
        </div>
      </div>
    </div>
  );
}
